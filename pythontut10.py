...
# are static variables. There value is shared by every ojbect of that class
...
class Dog:
    ...
    @staticmethod
    def getNumOfDogs():
        print("There are currently {} dog".format(Dog.num_of_dogs))
...
# ------------------- EXCEPTION HANDLING ----------------------
# Exception are triggered either when an error occurs
...
# Surrround a potential exception with tyr
try:
    aList = [1, 2, 3]
...
# with commas inside parenthesis
...
# 2. Using what you learned in part 8 and Google to find out how to open a file without with
#    try to open the file in a try block
# 3. Catch the FileNotFoundError exception
# 4. In else print the file contents
# 5. In finally close the file
# 6. Try to open the nonexistnent file mydata3.text and test to see if you caught the exception
