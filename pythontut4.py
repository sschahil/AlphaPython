...
# and then in parenthesis a comma separated list of values
# to pass if needed
...
# ------------- EXAMPLE 1 ----------------
# Variables created in a function can't be accessed outside of it
...
# Prints Tom even though the function to change it
print(name)
...
def isprime(num):
    # This for loop cycles through primes from 2 to the value to check
    for i in range(2, num):

        # If any division has no remainder we know it isn't a prime number
        if(num % i) == 0:
            return False
    return True
...
def getPrimes(max_number):
    ...
    # This for loop cycls through primes from 2 to the maximum value requested
...
# We can receive an unkonwn number of arguments using
# the splat (*) operator
...
# Functions allow us to avoid duplicate code in our program
...
def get_area(shape):
    ...
    if shape == "rectangle":
        ...
        print("Please enter rectangle or cirlce")
...
# Create a funciton that calculates the circle area
...
# We often place our main programming logic in a funciton called main
...
def main():
    ...
    # Call a funciton that will route to the correct function
    get_area(shape_type)
